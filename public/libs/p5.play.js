...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  */
  this.overlapPixel = function(pointX, pointY) {
    ...
    point.x -= this.position.x-img.frame.width/2;
    point.y -= this.position.y-img.frame.height/2;

    //out of the image entirely
    if(point.x<0 || point.x>img.frame.width || point.y<0 || point.y>img.frame.height)
      return false;
    else if(this.rotation === 0 && this.scale === 1)
    {
      //true if full opacity
      var values = this.animation.spriteSheet.image.get(point.x +img.frame.x,point.y + img.frame.y);
      return values[3] === 255;
    }
    else
    {
